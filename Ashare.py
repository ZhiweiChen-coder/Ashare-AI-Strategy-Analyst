...
def get_price(code, end_date='', count=10, frequency='1d', fields=[]):
    # 港股分支
    if code.lower().startswith('hk') or code.upper().endswith('.HK'):
        hkcode = code[2:] if code.lower().startswith('hk') else code
        if not hkcode.endswith('.HK'):
            hkcode = hkcode.zfill(5) + '.HK'
        txcode = 'hk' + hkcode.split('.')[0]
        print(f'using tencent hk: {txcode}')
        df = get_price_day_tx(txcode, count=count)
        print('tx df empty:', df.empty)
        if not df.empty:
            print('tx hk df head:', df.head())
            return df
        else:
            raise ValueError(f'腾讯接口未能获取到港股数据: {txcode}')

    # A股分支
    xcode = code.replace('.XSHG', '').replace('.XSHE', '').replace('.SH', '').replace('.SZ', '')
    xcode = 'sh' + xcode if ('.SH' in code or 'XSHG' in code) else 'sz' + xcode if ('.SZ' in code or 'XSHE' in code) else code
